...
public class RoundedCube : MonoBehaviour
{
    ...
    /*************************************************相 較 於 Cube.cs 多 的 部 分 ********************************************************/
    public int roundness;   //控制立方体的圆润化程度，范围应该在1到立方体最小尺寸的一半之间

    //放弃使用相邻相邻三角形的平均值，取而代之的是直接计算想要的圆弧度
    ...
    /*************************************************相 較 於 Cube.cs 多 的 部 分 ********************************************************/
    //外立方体上的任何一个顶点来说，在内立方体上都会有个最靠近它的顶点。可以通过它来定义圆弧立方体上的法线顶点
    //計算一個點的法線
    private void SetVertex(int i, int x, int y, int z)
    {
        ...
        //若 (設置的x座標 < 圆弧度) 則表示此內正方體上的點在左侧
        if (x < roundness)
            inner.x = roundness;
        //若 (設置的x座標 > x長度-圆弧度) 則表示此內正方體上的點在右侧
